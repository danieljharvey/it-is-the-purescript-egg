...
createFullCoord x y offsetX offsetY = Coord { x, y, offsetX, offsetY }

createMoveCoord :: Int -> Coord -> Coord
createMoveCoord speed (Coord coord) = createFullCoord 0 0 newX newY
  where
  newX = speed * coord.x
...
justOffset (Coord c) = Coord $ c { x = 0, y = 0 }
...
eqInts :: Coord -> Int -> Int -> Boolean
eqInts (Coord c) x y = c.x == x && c.y == y

directionCoord :: Coord -> Coord -> Coord
directionCoord (Coord current) (Coord aim) = createCoord x y
  where
