...
type GameLoop
  = (Int -> Maybe InputEvent -> GameState -> Maybe GameState)

-- old, new
type Listener
  = (GameState -> GameState -> Effect Unit)
...
animationLoop :: MutableState -> GameLoop -> Listener -> Effect Unit -> Effect Unit
animationLoop refs gameLoop listener completedCallback = do
  w <- window
  let
    callback = \c -> do
      ...
      let
        newGameState = gameLoop c newEvent gameState
      case newGameState of
        -- still playing
        Just newGameState' -> do
          Ref.write newGameState' refs.gameState
...
duration ref callback = do
  ...
  Ref.write time ref
  let
    change = difference oldTime time
  callback change
...
difference old new = millisecondsToInt (fromDuration myDiff)
  where
  myDiff :: Milliseconds
  myDiff = diff new old
